...
def max_fuel(graph: Dict[str, Chemical], given_ore: int = int(1e12)) -> int:
    # Assuming a DAG and monotonous function
    low, high = 0, given_ore
    max_ore_consumption, answer = 0, 0
    while low < high:
        mid = low + ((high - low) // 2)
        ore_consumed = ore_required(graph=graph, name='FUEL', desired_quantity=mid)
        if ore_consumed > given_ore:
            high = mid - 1
        else:
